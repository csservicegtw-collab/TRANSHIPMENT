...
export async function publishCargoRow(row){
  ...
  const agent = (row.agent || "").toString().trim().toUpperCase();
  const tsPort = (row.tsPort || "").toString().trim().toUpperCase();
  const routeKey = (row.routeKey || `${tsPort}_${agent}`).toUpperCase();
  ...
  await setDoc(doc(db, "cargo_tracking", bl), payload, { merge:true });
}

/* ✅ delete row from universal collection */
export async function deleteCargoRow(bl){
  const id = normalizeBL(bl);
  if(!id) return;
  await deleteDoc(doc(db, "cargo_tracking", id));
}
